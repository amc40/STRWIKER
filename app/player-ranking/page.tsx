...
const Page: React.FC = async () => {
  ...
  const maxOwnGoals =
    playersWithOwnGoals.length > 0 ? playersWithOwnGoals[0].ownGoalsCount : 0;
  ...
  const playersWithRankingAndGoals =
    playersOrderedByDescendingElosWithRanking.map((player) => ({
      ...player,
  ...
  const maxGoals = Math.max(
    ...playersWithRankingAndGoals.map((p) => p.totalGoals),
  );
